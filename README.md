...
| Pixie-Dust Downgrade       | WPS networks                | Off     | Makes the access point emit predictable WPS secret nonces so pixiewps recovers the PIN offline in seconds. Off, the AP resists Pixie Dust like a modern router and only falls to the slower online PIN brute force (reaver/bully).  |
| PMKID Exposed              | WPA2-Personal networks      | Off     | Makes the access point advertise the RSN PMKID in the first handshake frame so the passphrase is capturable clientlessly (hcxdumptool, then hashcat). Off, the AP withholds the PMKID and forces a full four-way-handshake capture. |
