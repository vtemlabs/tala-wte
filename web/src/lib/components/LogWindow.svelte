...
<script lang="ts">
  ...
  let { open = $bindable(false), title = 'Live Log', streaming = false, lines = [] }:
    { open?: boolean; title?: string; streaming?: boolean; lines?: string[] } = $props();

  let minimized = $state(false);
  let maximized = $state(false);
  let x = $state(0), y = $state(0), w = $state(860), h = $state(560);
  let started = false;
  ...
  $effect(() => {
    ...
    if (atBottom) requestAnimationFrame(() => { if (bodyEl) bodyEl.scrollTop = bodyEl.scrollHeight; });
  });
  ...
  $effect(() => {
    ...
    if (!open) { started = false; minimized = false; maximized = false; }
  });

  function close() { open = false; }
  function toggleMin() { minimized = !minimized; }
  function toggleMax() { if (minimized) { minimized = false; return; } maximized = !maximized; }
  function restore() { if (minimized) minimized = false; }
  ...
  function onMove(e: MouseEvent) {
    if (mode === 'drag') {
      ...
      if (mode === 'e' || mode === 'se') w = Math.max(360, Math.min(sW + e.clientX - sx, window.innerWidth - x - 8));
      if (mode === 's' || mode === 'se') h = Math.max(220, Math.min(sH + e.clientY - sy, window.innerHeight - y - 8));
    }
  }
  function onUp() {
    mode = '';
  ...
  <div class="tw" class:max={maximized} class:min={minimized}
    ...
    <div class="tw-title" onmousedown={(e) => start(e, 'drag')}
      ...
      <span class="tw-name">
        ...
        {title}{#if minimized} (click to restore){/if}
      </span>
      <div class="tw-win">
...
<style>
  ...
  .tw-name { display: inline-flex; align-items: center; gap: 8px; font-size: 12px; color: var(--text-muted); font-family: var(--font-mono, monospace); }
  .tw-led { width: 7px; height: 7px; border-radius: 50%; background: var(--text-dim); }
  .tw-led.on { background: var(--color-green); box-shadow: 0 0 8px var(--color-green); }
  ...
  .tw-body {
    ...
    background: var(--bg-primary); padding: var(--space-md) var(--space-lg);
  ...
  :global(.tw .tw-body::-webkit-scrollbar) { width: 8px; }
  :global(.tw .tw-body::-webkit-scrollbar-track) { background: transparent; }
  :global(.tw .tw-body::-webkit-scrollbar-thumb) { background: var(--border-primary); border-radius: 4px; }
  :global(.tw .tw-body::-webkit-scrollbar-thumb:hover) { background: var(--text-dim); }
</style>
