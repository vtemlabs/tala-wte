...
<script lang="ts">
  ...
  let { title, subtitle = '', backHref, backLabel = 'Back', crumb = '', doc }:
    { title: string; subtitle?: string; backHref: string; backLabel?: string; crumb?: string; doc: string } = $props();
</script>
...
<style>
  ...
  .prose :global(blockquote) {
    margin: 0 0 var(--space-md); padding: var(--space-sm) var(--space-lg);
    border-left: 3px solid var(--accent); background: var(--accent-soft);
    border-radius: var(--radius-sm); color: var(--text-secondary); font-size: var(--font-size-sm);
  }
