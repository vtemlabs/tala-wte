...
- **WPA (TKIP - Legacy)** / **WPA2-Personal (AES)** - passphrase, 8-63 characters. WPA2-Personal is the everyday choice for handshake-capture labs. Example: passphrase \`Summer2026!\` for a four-way-handshake exercise. By default the AP withholds the PMKID; turn on **PMKID Exposed** in Topology for a clientless PMKID capture lab.
...
- **PMKID Exposed** (WPA2-Personal only, off by default) - decides whether the WPA2 AP hands out its PMKID. **Off**, the AP withholds the PMKID like a modern router, so capturing the PSK needs a full four-way handshake from a real client. **On**, the AP advertises the RSN PMKID in the first handshake frame, so the PSK is capturable **clientlessly** (hcxdumptool, then crack with hashcat) - no connected client required. Turn it on to teach the clientless PMKID attack; leave it off to force a handshake capture.
- **Real WEP Beacon** (WEP networks only, on by default) - decides whether the WEP network actually broadcasts WEP. **On**, the AP uses the embedded WEP-capable hostapd and emits a real WEP beacon (Privacy bit), so clients WEP-associate and WEP key-recovery (IV collection, statistical crack) has a real target. **Off**, the AP uses the system hostapd, which on a modern OS is built without WEP and silently beacons the network **open** - clients cannot WEP-associate and there is nothing to attack. Leave it on for a working WEP lab; turn it off to demonstrate that a current AP cannot do WEP at all.
