...
<script lang="ts">
  ...
  const channelMap: Record<string, { ch: number; label: string }[]> = {
    '2.4': [
      { ch: 1, label: '1 (2412 MHz)' }, { ch: 2, label: '2 (2417 MHz)' }, { ch: 3, label: '3 (2422 MHz)' },
      { ch: 4, label: '4 (2427 MHz)' }, { ch: 5, label: '5 (2432 MHz)' }, { ch: 6, label: '6 (2437 MHz)' },
      { ch: 7, label: '7 (2442 MHz)' }, { ch: 8, label: '8 (2447 MHz)' }, { ch: 9, label: '9 (2452 MHz)' },
      { ch: 10, label: '10 (2457 MHz)' }, { ch: 11, label: '11 (2462 MHz)' },
    ],
    '5': [
      { ch: 36, label: '36 (5180 MHz)' }, { ch: 40, label: '40 (5200 MHz)' },
      { ch: 44, label: '44 (5220 MHz)' }, { ch: 48, label: '48 (5240 MHz)' },
      { ch: 52, label: '52 (5260 MHz) DFS' }, { ch: 56, label: '56 (5280 MHz) DFS' },
      { ch: 60, label: '60 (5300 MHz) DFS' }, { ch: 64, label: '64 (5320 MHz) DFS' },
      { ch: 100, label: '100 (5500 MHz) DFS' }, { ch: 104, label: '104 (5520 MHz) DFS' },
      { ch: 108, label: '108 (5540 MHz) DFS' }, { ch: 112, label: '112 (5560 MHz) DFS' },
      { ch: 116, label: '116 (5580 MHz) DFS' }, { ch: 120, label: '120 (5600 MHz) DFS' },
      { ch: 124, label: '124 (5620 MHz) DFS' }, { ch: 128, label: '128 (5640 MHz) DFS' },
      { ch: 132, label: '132 (5660 MHz) DFS' }, { ch: 136, label: '136 (5680 MHz) DFS' },
      { ch: 140, label: '140 (5700 MHz) DFS' }, { ch: 144, label: '144 (5720 MHz) DFS' },
      { ch: 149, label: '149 (5745 MHz)' }, { ch: 153, label: '153 (5765 MHz)' },
      { ch: 157, label: '157 (5785 MHz)' }, { ch: 161, label: '161 (5805 MHz)' },
      { ch: 165, label: '165 (5825 MHz)' },
    ],
    '6': [
      { ch: 1, label: '1 (5955 MHz)' }, { ch: 5, label: '5 (5975 MHz)' },
      { ch: 9, label: '9 (5995 MHz)' }, { ch: 13, label: '13 (6015 MHz)' },
      { ch: 17, label: '17 (6035 MHz)' }, { ch: 21, label: '21 (6055 MHz)' },
      { ch: 25, label: '25 (6075 MHz)' }, { ch: 29, label: '29 (6095 MHz)' },
      { ch: 33, label: '33 (6115 MHz)' }, { ch: 37, label: '37 (6135 MHz)' },
      { ch: 41, label: '41 (6155 MHz)' }, { ch: 45, label: '45 (6175 MHz)' },
      { ch: 49, label: '49 (6195 MHz)' }, { ch: 53, label: '53 (6215 MHz)' },
      { ch: 57, label: '57 (6235 MHz)' }, { ch: 61, label: '61 (6255 MHz)' },
      { ch: 65, label: '65 (6275 MHz)' }, { ch: 69, label: '69 (6295 MHz)' },
      { ch: 73, label: '73 (6315 MHz)' }, { ch: 77, label: '77 (6335 MHz)' },
      { ch: 81, label: '81 (6355 MHz)' }, { ch: 85, label: '85 (6375 MHz)' },
      { ch: 89, label: '89 (6395 MHz)' }, { ch: 93, label: '93 (6415 MHz)' },
    ]
  };
  ...
  const needsPassphrase = $derived(['wpa', 'wpa2', 'wps', 'wpa3', 'wpa3_transition', 'wep'].includes(protocol));
  const isWEP = $derived(protocol === 'wep');
  const isEnterprise = $derived(protocol.includes('enterprise'));
  ...
  async function save() {
    ...
    if (!ssid.trim()) { error = 'SSID Name is required'; window.scrollTo(0, 0); return; }
    if (new TextEncoder().encode(ssid).length > 32) { error = 'SSID must be 32 bytes or fewer'; window.scrollTo(0, 0); return; }
    if (!iface.trim()) { error = 'Wireless interface is required'; window.scrollTo(0, 0); return; }
    if (isWEP) {
      if (!normalizeWEPKey(passphrase)) { error = 'WEP key is required'; window.scrollTo(0, 0); return; }
    } else if (needsPassphrase) {
      if (passphrase.length < 8) { error = 'Passphrase must be at least 8 characters'; window.scrollTo(0, 0); return; }
      if (passphrase.length > 63) { error = 'Passphrase must be 63 characters or fewer'; window.scrollTo(0, 0); return; }
    }
    ...
    try {
      ...
      const rec = await networks.create({
        ssid, protocol, band, channel, passphrase: needsPassphrase ? effPassphrase : '',
        interface: iface, client_isolation: clientIsolation, internet_passthrough: internetPassthrough,
    ...
    <section class="panel">
      ...
      <div class="panel-body">
        <div class="form-group" style="margin-bottom:var(--space-lg)">
          ...
          <input class="input" id="ssid" bind:value={ssid} placeholder="e.g. TalaWTE-Target" maxlength="32" />
        </div>
        ...
        {#if needsPassphrase}
          <div class="form-group">
            <label class="field-label" for="passphrase">{isWEP ? 'WEP Key' : 'Passphrase (min. 8 chars)'}</label>
            <input class="input" id="passphrase" type="text" bind:value={passphrase} placeholder={isWEP ? 'any text, ASCII, or hex' : 'Network passphrase'} />
            {#if isWEP && wepEffective}
              <p class="field-desc">Effective key ({wepEffective.label}): <code style="color:var(--accent)">{wepEffective.key}</code> - enter this exact value on test clients.</p>
            {:else if isWEP}
              <p class="field-desc">WEP keys are 5 or 13 ASCII chars or 10/26 hex. Any other input is fitted to a valid 13-char key automatically.</p>
            {/if}
          </div>
        {/if}
      </div>
        ...
        <div class="form-grid" style="margin-bottom:var(--space-lg)">
          <div class="form-group">
            ...
            <select class="input" id="band" bind:value={band} onchange={onBandChange}>
              <option value="2.4" disabled={!bandSupported('2.4')}>2.4 GHz{bandSupported('2.4') ? '' : ' - not supported by this adapter'}</option>
              <option value="5" disabled={!bandSupported('5')}>5 GHz{bandSupported('5') ? '' : ' - not supported by this adapter'}</option>
              <option value="6" disabled={!bandSupported('6')}>6 GHz (Wi-Fi 6E){bandSupported('6') ? '' : ' - not supported by this adapter'}</option>
            </select>
        ...
        <div class="form-group">
          ...
          {#if interfaces.length}
            ...
            {#if Object.keys(inUse).length}
              <div class="field-desc" style="color:var(--text-muted)">
                In use by running networks (run a concurrent network on a free adapter): {Object.entries(inUse).map(([i, s]) => `${i} -> ${s}`).join(', ')}
              </div>
            {/if}
          {:else}
        ...
        {#if canHavePortal}
          <div class="toggle-field">
            <div>
              ...
              <div class="field-desc">Intercept unauthenticated traffic and serve a portal page</div>
            </div>
            ...
            <div class="form-group portal-nest">
              ...
              <select class="input" id="portalSelect" bind:value={selectedPortalHTML}>
                ...
                {#each portalsList as p}
                  <option value={p.html}>{p.name}{p.html.startsWith('fs:') ? ' (bundle)' : ''}</option>
                {/each}
              </select>
              <div class="toggle-field" style="margin-top:var(--space-sm)">
                <div>
                  <div class="toggle-name">Require Login (Directory / LDAP)</div>
                  <div class="field-desc">Validate submitted username and password against the directory before granting access, like a corporate or ISP hotspot. Failed logins are denied and recorded.</div>
                </div>
...
<style>
  ...
  .toggle-name { font-size: var(--font-size-sm); font-weight: 500; color: var(--text-primary); }

  .toggle-field .field-desc { margin-top: 2px; }
