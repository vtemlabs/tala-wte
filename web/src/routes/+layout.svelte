...
<script lang="ts">
  ...
  const ICONS: Record<string, string> = {
    ...
    networks: '<path d="M2 8.8a14 14 0 0 1 20 0"/><path d="M5 12.5a9 9 0 0 1 14 0"/><path d="M8.5 16a4.5 4.5 0 0 1 7 0"/><circle cx="12" cy="19.5" r="1"/>',
    portal: '<path d="M12 3l7 2.5V11c0 4.4-3 7.7-7 9-4-1.3-7-4.6-7-9V5.5L12 3z"/><path d="M9 12l2 2 4-4"/>',
    ...
    radius: '<circle cx="8" cy="14" r="4"/><path d="M10.8 11.2 20 2"/><path d="m16 6 2.5 2.5"/><path d="m14 8 2.5 2.5"/>',
    ...
    terminal: '<rect x="3" y="4" width="18" height="16" rx="2"/><path d="m7.5 9.5 3 2.5-3 2.5"/><path d="M13 15h4"/>',
    logout: '<path d="M14 4h3a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2h-3"/><path d="M10 12H3"/><path d="m6.5 8-4 4 4 4"/>',
  };

  onMount(async () => {
    try { collapsed = localStorage.getItem('sidebarCollapsed') === '1'; } catch { /* ignore */ }
    const currentPath = window.location.pathname;
    if (currentPath === '/login') { authChecked = true; return; }
...
{#if !authChecked}
  <div style="display:flex;align-items:center;justify-content:center;min-height:100vh;background:var(--bg-primary);color:var(--text-dim)">
    Loading...
  </div>
{:else if isLoginPage}
  {@render children()}
{:else}
  <div class="app-shell" class:sidebar-collapsed={collapsed}>
    ...
    {#if sidebarOpen}
      <button class="sidebar-backdrop" onclick={() => sidebarOpen = false} aria-label="Close menu" tabindex="-1"></button>
    {/if}

    {#snippet ic(name: string)}
      <svg class="nav-ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">{@html ICONS[name] ?? ''}</svg>
    {/snippet}
    ...
    <nav class="sidebar" class:open={sidebarOpen}>
      ...
      <div class="sidebar-nav">
        <a href="/" class="nav-item" class:active={isActive('/')} onclick={navClick} title="Dashboard">{@render ic('dashboard')}<span>Dashboard</span></a>
        <div class="nav-group-label">Networks</div>
        <a href="/networks" class="nav-item" class:active={isActive('/networks')} onclick={navClick} title="Networks">{@render ic('networks')}<span>Networks</span></a>
        <a href="/portals" class="nav-item" class:active={isActive('/portals') && !page.url.pathname.startsWith('/portals/captured')} onclick={navClick} title="Captive Portals">{@render ic('portal')}<span>Captive Portals</span></a>
        <div class="nav-group-label">Monitoring</div>
        <a href="/captures" class="nav-item" class:active={isActive('/captures')} onclick={navClick} title="Captures">{@render ic('captures')}<span>Captures</span></a>
        <a href="/portals/captured" class="nav-item" class:active={isActive('/portals/captured')} onclick={navClick} title="Captured Data">{@render ic('data')}<span>Captured Data</span></a>
        <div class="nav-group-label">Enterprise</div>
        <a href="/ldap" class="nav-item" class:active={isActive('/ldap')} onclick={navClick} title="LDAP Directory">{@render ic('ldap')}<span>LDAP Directory</span></a>
      ...
      <div class="sidebar-footer">
        <button class="nav-item logout-btn" onclick={() => { terminalOpen = true; sidebarOpen = false; }} title="Terminal">{@render ic('terminal')}<span>Terminal</span></button>
        <button class="nav-item logout-btn" onclick={() => { pb.authStore.clear(); goto('/login'); }} title="Logout">{@render ic('logout')}<span>Logout</span></button>
        <button class="nav-item logout-btn collapse-btn" onclick={toggleCollapse} title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'} aria-label="Toggle sidebar width">
...
<style>
  ...
  .hamburger {
    ...
    border-radius: 6px; color: var(--text-primary); font-size: 20px;
  ...
  .sidebar {
    width: 236px; background: var(--bg-secondary);
  ...
  .sidebar-sub { font-size: 10px; color: var(--text-dim); letter-spacing: 0.04em; text-transform: uppercase; }

  .sidebar-nav { flex: 1; padding: var(--space-md) var(--space-md); overflow-y: auto; }
  ...
  .nav-divider { height: 1px; background: var(--border-primary); margin: var(--space-sm) var(--space-md); }
  ...
  :global(.nav-ic) { width: 18px; height: 18px; flex-shrink: 0; opacity: 0.85; }
  .nav-item:hover { color: var(--text-primary); background: var(--bg-hover); }
  .nav-item.active {
    color: var(--text-primary); background: var(--accent-soft);
    box-shadow: inset 0 0 0 1px rgba(47, 129, 247,0.22);
  }
  .nav-item.active :global(.nav-ic) { opacity: 1; color: var(--accent-hover); }
  .nav-item.active::before {
    content: ""; position: absolute; left: -1px; top: 7px; bottom: 7px; width: 3px;
    border-radius: 0 3px 3px 0; background: var(--accent); box-shadow: 0 0 8px var(--accent-glow);
  }
  ...
  @media (min-width: 901px) {
    ...
    .app-shell.sidebar-collapsed .content { margin-left: 64px; }
    .app-shell.sidebar-collapsed .sidebar-header { align-items: center; padding: var(--space-lg) 0; }
    ...
    .app-shell.sidebar-collapsed .brand-icon { display: block; }
    .app-shell.sidebar-collapsed .nav-item { justify-content: center; padding: 9px 0; gap: 0; }
    .app-shell.sidebar-collapsed .nav-divider { margin: var(--space-sm) var(--space-md); }
  }

  @media (max-width: 900px) {
    .hamburger { display: flex; }
    .sidebar-backdrop { display: block; position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 99; }
    .sidebar { transform: translateX(-100%); transition: transform 0.2s ease; width: 240px; z-index: 200; }
    .sidebar.open { transform: translateX(0); }
    .content { margin-left: 0; padding: var(--space-lg); padding-top: 60px; }
  }
</style>
